...
def detect_activities(df, smoothing_window_size=5, walk_pace_threshold=9.0, cadence_threshold=140, min_segment_duration=30):
    ...
    for segment in merged_segments:
        if len(final_segments) == 0:
            ...
            if segment['activity_type'] == last_segment['activity_type']:
                last_segment['end_time'] = segment['end_time']
                last_segment['distance'] += segment['distance']
                last_segment['duration'] += segment['duration']
            else:
                final_segments.append(segment)
    segments = pd.DataFrame(final_segments)
    return segments
